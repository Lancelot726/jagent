...
| 机制 | 在程序里是什么 | 怎么看 |
|---|---|---|
| **博弈论** | 密封拍卖决定本轮用廉价档还是强力档；中标按第二高价（Vickrey）结算 | 每轮打印 `pick cheap=0.44 strong=0.51 -> strong (pay 0.44, +0.14)` |
| **经济学** | token 预算账本 + 压缩的 ROI 判定（不是到阈值就压） | 打印 `compress 182 -> 16 tok ... ROI 2116.5x` 与预算行 |
...
除了这一屏，运行过程只打印 `step` / `tool` / `result` 三类行；工具参数只摘要显示路径或命令本身，不打印原始 JSON。机制对账块（`summary` 那八行）**默认不打印**，需要时加 `--scorecard`。
...
| `--scorecard` | 在运行末额外汇总机制对账块（默认关闭，只给运维/调试看） |
| `--cwd <dir>` | 工作根目录 |
...
| `P7T` | 109 | 双厂商机制真实验证：竞价、声誉漂移、控制器升降与跳闸、降级链、对账聚合 |
...
**R3 本轮**：终端交互与显示口径。除「不带参数启动进交互式命令行、执行图只画结构与状态、机制标签中性化」外，另并入现场反馈的四项：把本机环境事实（os / shell / cwd / console encoding）注入系统提示；`bash` 工具关闭子进程 stdin 并按控制台编码解码，使 `cmd.exe` 的内建命令不再挂死、中文不再乱码；`runs.md` 让「本地记忆」跨进程成立，同一会话内的上下文也不再丢；终端只显示剥掉 Markdown 记号的纯文本，且每次运行——无论成败——都在同一段 `answer` 里收尾（原先另起的「本轮总结」块与机制对账块已删掉，后者改为 `--scorecard` 按需索取）。步数上限由写死的 16 变成配置项 `--max-steps`（根与子 agent 同用），设 `0` 即不限步数。

**R4 待触发**：GraalVM native 构建（`pom.xml` 已预留 `native` profile）、分层流程图布局、终端 raw mode 行编辑（方向键历史等）。进化论项暂缓，无期限。
