...
| 闸门 | 内容 | 状态 |
|---|---|---|
| G1 口径确认 | 范围 / 第二厂商归属 / 密钥投喂方式 | **已过**（初判"双模型接入+拍卖验证"，作者随后追加至**四组机制全测**，见修订 2b；厂商由作者自定且产品**厂商无关**；密钥由作者写入 `work/jagent.properties`） |
| G2 缺料求援 | 两个厂商的 `base_url` / `api_key` / `model` | **已过**（作者已投喂两档真实配置到 `work/jagent.properties`，两档分属两家） |
| G3 人工送审 | 作者在本机真终端实跑真实厂商；收尾看"机制对账"块即可判定四组机制是否生效 | **已过**（五个真实场景 d1–d5 全部跑通；首回暴露 3 处缺陷，退回 S5 修完后第二回通过） |
...
| P8 | 机制对账单（`Scorecard` + `work/scorecard.txt`） | **完成** |
| P9 | 断言 P7T（109 条） | **完成** |
| P10 | mock 电池：A 基线 / B 预算压力 / C 压缩 / D 控制论 / E 故障注入 | **完成** |
| P11 | 真实厂商最简 agent 跑通 | **完成**（d1：3 步 6 节点，写入并读回成功） |
| P12 | 真实厂商全机制对账证据固化 | **完成**（d1–d5 五场景，见下） |
...
| 机制 | 证据 | 结论 |
|---|---|---|
| 博弈论 拍卖 | A 场景 10 次竞价、强力档 10:0；B 场景加预算压力后翻转为 5:5 | **活**，随预算压力改变选择 |
| 博弈论 信誉 | E 场景失败后强力档信誉 1.00 -> 0.75 | **活**，事后结算生效 |
| 经济学 预算 | B 场景 2258/1000 (226%)；A 场景 2258/40000 (6%) | **活** |
| 经济学 压缩 ROI | C 场景 3 次压缩、省 518 tok；A/B/D 均 0 次（未达阈值） | **活** |
| 工程经济学 利润 | A +1.38；B 预算压力下转为 -1.23 | **活**，可与预算同屏对账 |
| 控制论 闭环调参 | D 场景 `--permits 1` 起手，上调 7 次至上限 8（峰值 6→2，见下方语义变更） | **活** |
| 控制论 熔断 | E 场景 熔断 1 次、下调 2 次、错误率 49% | **活** |
| 图工程 | A 26 节点全完成；C 因压缩多 3 节点；E 失败 2 | **活** |
...
| 场景 | 内容 | 结果 |
|---|---|---|
| d1 基线 | 写文件并读回 | 3 步 6 节点，强力档中标，成功 |
| d2 预算压力 | `--budget 1200` | **拍卖真翻转**：轮1 强力档 0.44/0.51；轮2 廉价档 0.18/0.15 中标；轮3 廉价档。两档各自真实执行了工具调用 |
...
| d5 故障注入 | 真实 429 限流 | 熔断 1、下调 2、重试 1、错误率 49%；强力档信誉 1.00→0.75；本轮中止 |

| 机制 | 真实厂商证据 | 判定 |
|---|---|---|
| 博弈论 拍卖 | d2 同一次运行内先强力档后廉价档；d3 共 14 次、廉价 3 / 强力 11 | **活** |
| 博弈论 信誉 | d3 廉价档 1.00 -> 0.75（失败结算），强力档 0.98；d5 强力档 1.00 -> 0.75 | **活** |
| 经济学 预算 | d2 347%；d3 57%；d4 5% | **活** |
| 经济学 压缩 ROI | d4 1 次、省 211 tok、ROI 1743.5x | **活** |
| 控制论 闭环+熔断 | d3 峰值 2、上调 9、下调 3、熔断 1、错误率 2%；d5 熔断 1、下调 2、错误率 49% | **活** |
| 降级/重试 | d3 备用档切换 12 次、重试 2 次；d5 重试 1 次 | **活** |
| 图工程 | d3 37 节点三态齐备；d5 失败 2 | **活** |
