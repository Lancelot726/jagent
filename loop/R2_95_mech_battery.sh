...
run() {
  ...
  java -jar "$JAR" run "$name" --cwd "$ROOT/$name" --no-graph "$@" 2>&1 \
    | strip \
    | grep -E '报价|步数|档位选择:|成功率:|压缩:|自适应并发控制:|降级:|执行图:|路径互斥:'
